...
namespace InlaksIB
{
    public class PostgreSqlWarehouse : WarehouseInterface
    {
         public  List<ValuePair> getFilterColumns(string tablename)
        {
            ...
            var pairs = db.getValuePair("ordinal_position", "column_name", "select ordinal_position, column_name from INFORMATION_SCHEMA.COLUMNS where table_name = '"+tablename+"' order by ordinal_position;");
